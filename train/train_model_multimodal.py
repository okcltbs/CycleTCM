"""
Train CycleTCM (multimodal) for tongue image classification.
8-class syndrome and 5-class organ prediction with weighted BCE; fuses Qwen MLLM features.
"""
...
from models.model_multimodal import CycleTCM, count_parameters, print_model_info
...
def load_mllm_feature_index(mllm_features_file: str) -> dict[str, list[float]]:
    """Map image_file basename -> qwen_feature vector from all_features.json."""
    ...
    for rec in records:
        ...
        index[key] = qwen_feature
    return index


def _image_key_for_mllm(img_whole_path: str, sample: dict) -> str:
    """Resolve lookup key: prefer sample image_file, else basename of img_whole."""
    if sample.get("image_file"):
        return os.path.basename(sample["image_file"])
    return os.path.basename(img_whole_path)
...
class TongueImageDataset(Dataset):
    ...
    def __init__(
        ...
        transform=None,
        id_list=None,
        mllm_features_file=None,
    ):
        self.feature_file = feature_file
        self.base_dir = base_dir if base_dir else os.path.dirname(feature_file)
        self.transform = transform
        self.mllm_by_image: dict[str, list[float]] = {}
        if mllm_features_file:
        ...
        for sample in self.features:
            ...
            if all_exist:
                image_key = _image_key_for_mllm(img_whole_path, sample)
                if self.mllm_by_image and image_key not in self.mllm_by_image:
                    logger.warning(
                        f"Sample {sample_id} missing MLLM feature for {image_key}, skipped"
                    )
                    continue
    ...
    def __getitem__(self, idx):
        ...
        mllm_feature = torch.tensor(
            self.mllm_by_image[image_key], dtype=torch.float32
        )
    ...
    for batch_idx, batch in enumerate(pbar):
        try:
            ...
            syndrome_outputs, organ_outputs = model(
                ...
                img_spleen, img_liver, img_kidney, mllm_feature
            )
...
def predict_test(model, config, device):
    ...
    mllm_features_file = config.get('mllm_features_file')
    ...
    test_dataset = TongueImageDataset(
        feature_file,
        base_dir=base_dir,
        transform=test_transform,
        id_list=test_ids,
        mllm_features_file=mllm_features_file,
    )
...
def train_model(config):
    ...
    """
    logger.info(f"\n{'='*80}")
    logger.info(f"Starting CycleTCM multimodal training (weighted BCE, 7 images + MLLM, syndrome + organ)")
    logger.info(f"{'='*80}")
    ...
    train_dataset = TongueImageDataset(
        feature_file,
        base_dir=base_dir,
    ...
    val_dataset = TongueImageDataset(
        feature_file,
        base_dir=base_dir,
...
def main():
    ...
    logger.info("="*80)
    logger.info("Starting CycleTCM multimodal (7 images + MLLM qwen_feature, weighted BCE)")
    logger.info("Feature file: feature_all_encoded.json")
