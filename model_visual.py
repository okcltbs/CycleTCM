"""
Visual Model, with AGLFF and UWBMoE.
Used for visual classification, and future late-fusion strategy.
"""
...
class CycleTCM(nn.Module):
    def __init__(self, num_classes1=8,num_classes2=5, pretrained=False, **kwargs):
        ...
        self.bn_syndrome = nn.BatchNorm1d(embedding_dim * 2)
        self.dropout_syndrome = nn.Dropout(0.3)
        self.bn_organ = nn.BatchNorm1d(embedding_dim * 2)
        self.dropout_organ = nn.Dropout(0.3)

        self.classifier1 = nn.Linear(embedding_dim * 2 , num_classes1)

        self.classifier2 = nn.Linear(embedding_dim * 2 , num_classes2)
    ...
    def forward(self, x_whole, x_edge, x_body, x_heart_lung, x_spleen, x_liver, x_kidney):
        ...
        print(x_syndrome_all.shape, x_organ_all.shape)
...
def _smoke_test() -> None:
    """Random-input forward pass; no ImageNet pretrained weights."""
    torch.manual_seed(0)
    ...
    print("CycleTCM (visual) smoke_test ok")
